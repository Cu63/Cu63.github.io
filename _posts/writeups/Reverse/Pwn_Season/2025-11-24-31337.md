...
Первая моя мысль - попробовать вызвать функцию `obfuscated_give_flag(int)`. Об этом я уже [писал ранее](https://cu63.github.io/reverse/call/).
...
Можно так же попатчить память через отладчик. Думаю, что можно было бы и пропатчить бинарь, чтобы в функцию всегда передавалось значение `31337`. Но особого смысла это не имеет. Подробнее о таком можно посмотреть [вот тут](https://cu63.github.io/reverse/task1/).
